...
def vypis_oznam(data):
    print(f"{data['name']} ma {data['follower_count']} sledujicich")
    
def vypis_dotaz(data):
    volba = input(f'{data["name"]} má podle tebe více H nebo méně L sledujících\n')
    return volba
...
while pokracuj:
    ...
    bod = porovnani(data1,data2,volba)
    body = body + bod
